...
class KittiEvalOdom():
    ...
    def eval(self, gt_dir, result_dir, 
        ...
        """
        dataset = Path(gt_dir).parent.name   # .../kitti/gt_poses/ -> parent is kitti
        seq_dir = sorted(glob(os.path.join(gt_dir, "*.txt")))
        seq_list = [Path(i).stem for i in seq_dir]
        ...
        if seqs is None:
            ...
            self.eval_seqs = [Path(i).stem for i in available_seqs if Path(i).stem in seq_list]
